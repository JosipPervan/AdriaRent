                ...
                <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                    <form method="POST" action="{{ route('admin.offers.update', $offer->id) }}">
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <div class="mt-1">
                                <input type="number" min="0.00" max="10000.00" id="price"  name="price" value="{{ $offer->price }}"
                                       class="block w-full transition duration-150 ease-in-out appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                            </div>
                            @error('price')
                            <div class="text-sm text-red-400">{{ $message }}</div>
                            ...
                            <div class="mt-1">
                                <select id="status" name= "status" class="form-multiselect block w-full mt-1">
                                    @foreach(App\Enums\OfferStatus::cases() as $status)
                                        <option value="{{$status->value}}" @selected($offer->status->value == $status->value)>{{ $status->name }} </option>
                                    @endforeach
                                </select>
                            </div>
                            @error('status')
