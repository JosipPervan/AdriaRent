            ...
            <div class="m-2 p-2 rounded">
                <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                    <form method="POST" action="{{ route('admin.equipments.update', $equipment->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="sm:col-span-6">
                            <label for="image" class="block text-sm font-medium text-gray-700"> Slika </label>
                            <div>
                                <img class="w-20 h-20" src="{{ Storage::url($equipment->image) }}">
                            </div>
                            <div class="mt-1">
                        ...
                        <div class="sm:col-span-6 ">
                            <label for="categories" class="block text-sm font-medium text-gray-700">Kategorije</label>
                            <div class="mt-1">
                                <select id="categories" name="categories[]" class="form-multiselect block w-full mt-1"
                                    multiple>
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" @selected($equipment->categories->contains($category))> {{ $category->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="m-6 p-4">
                            <button type="submit" class="px-4 py-2 bg-indigo-500 hover:bg-indigo-700 rounded-lg text-white">Promjeni</button>
                        </div>
                    </form>
                </div>
